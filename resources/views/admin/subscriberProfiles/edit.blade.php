...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.subscriberProfile.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.subscriber-profiles.update", [$subscriberProfile->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('package') ? 'is-invalid' : '' }}" name="package_id" id="package_id" required>
                    @foreach($packages as $id => $entry)
                        <option value="{{ $id }}" {{ (old('package_id') ? old('package_id') : $subscriberProfile->package->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('business_name') ? 'is-invalid' : '' }}" type="text" name="business_name" id="business_name" value="{{ old('business_name', $subscriberProfile->business_name) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('business_address') ? 'is-invalid' : '' }}" name="business_address" id="business_address">{{ old('business_address', $subscriberProfile->business_address) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('company_registration_number') ? 'is-invalid' : '' }}" type="text" name="company_registration_number" id="company_registration_number" value="{{ old('company_registration_number', $subscriberProfile->company_registration_number) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('subdomain_name') ? 'is-invalid' : '' }}" type="text" name="subdomain_name" id="subdomain_name" value="{{ old('subdomain_name', $subscriberProfile->subdomain_name) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('contact_number') ? 'is-invalid' : '' }}" type="text" name="contact_number" id="contact_number" value="{{ old('contact_number', $subscriberProfile->contact_number) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\SubscriberProfile::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $subscriberProfile->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('account_type') ? 'is-invalid' : '' }}" name="account_type" id="account_type" required>
                    ...
                    @foreach(App\Models\SubscriberProfile::ACCOUNT_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('account_type', $subscriberProfile->account_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('remaining_trial_period') ? 'is-invalid' : '' }}" type="number" name="remaining_trial_period" id="remaining_trial_period" value="{{ old('remaining_trial_period', $subscriberProfile->remaining_trial_period) }}" step="1" required>
