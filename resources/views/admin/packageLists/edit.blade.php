...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.packageList.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.package-lists.update", [$packageList->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="package_name">{{ trans('cruds.packageList.fields.package_name') }}</label>
                <input class="form-control {{ $errors->has('package_name') ? 'is-invalid' : '' }}" type="text" name="package_name" id="package_name" value="{{ old('package_name', $packageList->package_name) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{{ old('description', $packageList->description) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('package_price') ? 'is-invalid' : '' }}" type="number" name="package_price" id="package_price" value="{{ old('package_price', $packageList->package_price) }}" step="0.01" required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('default_selected') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="default_selected" id="default_selected" value="1" {{ $packageList->default_selected || old('default_selected', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('currency') ? 'is-invalid' : '' }}" name="currency" id="currency">
                    ...
                    @foreach(App\Models\PackageList::CURRENCY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('currency', $packageList->currency) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('color_code') ? 'is-invalid' : '' }}" type="text" name="color_code" id="color_code" value="{{ old('color_code', $packageList->color_code) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('trial_periods') ? 'is-invalid' : '' }}" name="trial_periods" id="trial_periods" required>
                    ...
                    @foreach(App\Models\PackageList::TRIAL_PERIODS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('trial_periods', $packageList->trial_periods) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
