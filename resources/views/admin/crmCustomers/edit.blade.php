...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.crm-customers.update", [$crmCustomer->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="subscriber_id">{{ trans('cruds.crmCustomer.fields.subscriber') }}</label>
                <select class="form-control select2 {{ $errors->has('subscriber') ? 'is-invalid' : '' }}" name="subscriber_id" id="subscriber_id" required>
                    @foreach($subscribers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('subscriber_id') ? old('subscriber_id') : $crmCustomer->subscriber->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('subscriber'))
                    <span class="text-danger">{{ $errors->first('subscriber') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.crmCustomer.fields.subscriber_helper') }}</span>
            </div>
            <div class="form-group">
